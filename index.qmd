...
## Verbal definition of a node <!--# Should this be Add Node: Node Definition - Not sure what it means Verbal  -->
...
<!--# The screeen should have Add Node instead of Add new item -->
...
### Example for markdown syntax (Alizarin red S in Eel) <!--# move this after read more? or as another Tip callout? -->
...
Several options are available for defininig the mathematical properties of a node, including user defined, parametric distribution, bootstrapping and EFSA EKE. <!--# maybe change EFSA EKE to just expert elicitation -->
...
### Parametric distributions <!--# Maybe need to re-consider this name since some of the distributions are empirical and therefore non-parametric (cumulative, histogram, general) -->

::: {.callout-note appearance="minimal"}
Upon completing this section, you will master the use of probability distributions in in shiny rrisk, a prerequisite for reflecting variability and uncertainty in your model. <!--# variability and uncertainty can be reflected in general under the mathematical definition of a node -->
:::
...
Please refers to this compendium of distributions to select the apprpriated distributions and distribution parameters: [Compendium of Distribution](https://www.vosesoftware.com/knowledgebase/whitepapers/pdf/ebookdistributions.pdf) <!--# we could download the pdf so we will be sure that always it is there, in case website changes -->
...
# Load YAML
 yaml_data <- yaml::read_yaml("terms.yaml")

# Add anchor tags for linking
glossary_df <- data.frame(
  Term = sapply(yaml_data, function(x) 
    paste0('<a name="', tolower(gsub("[^a-zA-Z0-9]", "-", x$term)), '">', x$term, '</a>')),
  Definition = sapply(yaml_data, function(x) x$definition),
  stringsAsFactors = FALSE
)

DT::datatable(glossary_df, escape = FALSE)
