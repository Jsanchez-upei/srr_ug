...
Documentation of the **Seed value** allows you to exactly reproduce the numerical results of your model. This is possible because the random number generation uses a fully deterministic process. However, for practical purposes, the numbers generated using both the MC and LH algorithms can be assumed to be random, independent and identically distributed.

The default setting of the **Type** for calculating percentiles follows the recommendation given by the developers of R's "quantile" function (see [see documentation](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/quantile)), which is part of the R "stats" package (R Core Team, 2024. R: A Language and Environment for Statistical Computing. R Foundation for Statistical Computing, Vienna, Austria. <https://www.R-project.org/>).
...
For example, to observe at least $n=5$ events with a $p = 1/10000=10^{-4}$, you will need $N\ge=5*10^4$, at least 50,0000 samples. Finding the appropriate number of samples is an iterative process, where different simulations are run until consistent values for up to two/three digits of the mean or median are obtained. The second and more general approach for assessing the required number of replicates uses the **convergence plot** (see below).  The convergence plot displays the numerical result of you model as a function of the number of replicates. Therefore, you can use this approach also if some other metric than number of adverse events is the outcome of your model.
:::

::: {.callout-tip collapse="true"}
### Good practice - choice of percentiles for numerical results

The median  (as a measure of central tendency) and 95th percentile (as an upper are usually  reported to characterize the variability  of the outcome in exposure or  risk models. If a 2D simulation has been conducted to account for (parameter) uncertainty, the outcome statistics are usually reported with a 95% uncertainty interval, i.e. an interval ranging from the 2.5th to the 97.5th percentile, [0.025, 0.975] in decimal notation of the uncertainty distribution. Other symmetrical uncertainty intervals such  
:::
...
![Numerical results for a one-dimensional (1D) simulation (*Yersinia enterocolitica* in minced meat model).](images/general_results_out_1D.png){#fig-general_results_out_1D width="100%"}
...
Assume you were interested in the high, say 95th percentile of the outcome distribution. This is typically the case to account for the high end of the range of outcomes consistent with the information captured in the model. Using a **deterministic approach**, you would use the "high risk percentile" for each of the input variables. Note that a "high risk" percentile could be the 5th percentile if the quantity is in the denominator of the risk expression. The result of the  

::: {.callout-caution collapse="true"}
## Read more - Monte Carlo (MC) vs. latin hypercube (LH) sampling

Two sampling methods can be used in shiny rrisk. The Latin Hypercube (LH)  you would would evaluate  te mathematical in the pop,     
:::
